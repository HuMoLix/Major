...
│   ├── setup_wireguard.sh    # Ubuntu 自动化安装配置 WireGuard 脚本
...
## 第一部分：服务端部署 (Ubuntu 24.04)

### 1. 系统准备与防火墙配置
在 Ubuntu 机器上，更新软件包并开放所需的端口：
- **8000/TCP**：FastAPI 客户端激活服务
- **8080/TCP**：Flask 网页管理员后台
- **51820/UDP**：WireGuard 虚拟隧道端口
- **22/TCP**：SSH 连接（确保不要把自己锁在外面）
...
# 初始化数据库并自动迁移表结构 (添加 is_banned 和 duration_seconds)
python init_db.py
```
...
# 启用并开启开机自启
sudo systemctl enable vpn-api.service vpn-web.service
sudo systemctl start vpn-api.service vpn-web.service

# 检查运行状态
sudo systemctl status vpn-api.service vpn-web.service
```
...
4. **全流量代理配置**（如果客户端处于无默认网关的隔离局域网中，请执行以下命令设置默认流量走 VPN）：
   ...
   # 格式: route add 0.0.0.0 mask 0.0.0.0 <已分配的隧道IP>
...
#### 3. 封禁激活码并强制下线 (Ban)
如果发现激活码违规或用户欠费：
- 在列表中点击该激活码操作列 of **Ban** 按钮。
- **即时断网**：在修改数据库的同时，服务端会立即向系统内核发送 Peer 剔除指令，客户端的 VPN 隧道将瞬时失效，断开数据流通讯。
